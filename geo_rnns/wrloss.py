...
class WeightMSELoss(Module):
    def __init__(self, batch_size, sampling_num):
        ...
        self.weight = Parameter(torch.Tensor(self.weight.astype(float)).cuda(), requires_grad = False)  # self.weight->self.weight.astype(float)
...
class WeightedRankingLoss(Module):
    ...
    def forward(self, p_input, p_target, n_input, n_target):
        trajs_mse_loss = self.positive_loss(p_input, autograd.Variable(p_target).cuda(), False)  # 相似样本的loss

        negative_mse_loss = self.negative_loss(n_input, autograd.Variable(n_target).cuda(), True) # 不相似样本的loss
