...
class SAM_LSTMCell(RNNCellBase):
    def __init__(self, input_size, hidden_size, grid_size, bias=True, incell = True):
        ...
        self.bias = bias
        self.weight_ih = Parameter(torch.Tensor(5 * hidden_size, input_size-2))  # 5个gate的输入权重矩阵
        self.weight_hh = Parameter(torch.Tensor(5 * hidden_size, hidden_size))   #5个gate的上一个隐层输出权重矩阵
        if bias:
            ...
            self.register_parameter('bias_ih', None)
            self.register_parameter('bias_hh', None)
        self.spatial_embedding = SpatialExternalMemory(grid_size[0]+3*config.spatial_width,              # 设置memory的大小
    ...
    def spatial_lstm_cell(self, input_a, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
        ...
        input = input_a[:,:-2]    #坐标
        grid_input = input_a[:,-2:].type(torch.LongTensor).cuda() + config.spatial_width  # grid index

        hx, cx = hidden
        gates = F.linear(input, w_ih, b_ih) + F.linear(hx, w_hh, b_hh)  # input:[220,2] hx:[220,128]
        ...
        atten_cs, attn_weights = self.atten(cy_hh,cs)  # attention操作
        c = cy_h + spatialgate * atten_cs  # 总的cell state
        # c = cy_h
        hy = outgate * F.tanh(c)  # ht

        if self.incell:
            grid_x, grid_y = grid_input[:, 0].data, grid_input[:, 1].data
            self.sg = spatialgate.data
            self.c_d = c.data
            updates = self.sg* self.spatial_embedding.read(grid_x, grid_y) + (1-self.sg) * self.c_d  # 更新memory
            if self.training:
