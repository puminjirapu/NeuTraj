...
class RNNEncoder(Module):
    ...
    def forward(self, inputs_a, initial_state = None):
        ...
        outputs = []
        for t in range(time_steps): # time_step被设定为最长的轨迹长度
            if self.stard_LSTM:
                cell_input = inputs[:, t, :][:,:-2]
            else:
                cell_input = inputs[:, t, :]      # inputs的维度为(220,145,4)，提取出某一个time_step [220,4]
            if config.recurrent_unit == 'GRU' or config.recurrent_unit == 'SimpleRNN':
                out = self.cell(cell_input, out)
            else:
                out, state = self.cell(cell_input, (out, state))   # out=ht ， state=cell state
            outputs.append(out)   # time_step走完之后，outputs的维度是(145,220,128)
        mask_out = []
        for b, v in enumerate(inputs_len):
            mask_out.append(outputs[v-1][b,:].view(1,-1))   # 根据轨迹长度进行mask操作，即提取出相应time_step的hidden层结果
        return torch.cat(mask_out, dim = 0)   # (220,128)
...
class NeuTraj_Network(Module):
    ...
    def forward(self, inputs_arrays, inputs_len_arrays):
        ...
        return trajs_loss, negative_loss     # 返回anchor与positive和negative嵌入之后的相似度距离
